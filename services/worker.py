...
def process_account_job(uid: str, job_id: str) -> dict | None:
    started = time.monotonic()
    claimed = claim_account_job(uid, job_id)
    if not claimed or claimed.get("status") != "running":
        return claimed
    _, data = get_account_job_internal(uid, job_id)
    logging.info("job_started job_id=%s uid=%s kind=%s attempt=%s",
                 job_id, uid, data.get("kind"), data.get("attempts"))
    try:
        kind = data.get("kind")
        if kind == "account_export":
            result = run_account_export(uid, job_id, data)
        else:
            raise ValueError(f"unsupported account job kind: {kind}")
        completed = update_account_job(uid, job_id, status="succeeded", progress=100,
                                       stage="complete", result=result, error=None)
        logging.info("job_succeeded job_id=%s kind=%s duration_ms=%s", job_id, kind,
                     round((time.monotonic() - started) * 1000))
        return completed
        ...
        logging.exception("job_failed job_id=%s kind=%s duration_ms=%s", job_id,
                          data.get("kind"), round((time.monotonic() - started) * 1000))
        error = {"code": "job_failed", "message": str(exc)[:500]}
        return update_account_job(uid, job_id, status="failed", stage="failed", error=error)
