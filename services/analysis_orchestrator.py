"""Structured case analysis, run inside the async job worker.

Shared by /analyze and /intake (routes/analyze.py, routes/intake.py), which
both queue a "matter_analysis" job against a matter's description -- the
only difference between the two routes is what populates that description
before the job is queued. Mirrors chat_orchestrator.py's pattern: this runs
outside Flask request context, so it reads/writes matter state through
services/matters.py directly rather than models/context.py's
FirestoreBackedDict, which is only safe to use from an in-request route.
"""
from __future__ import annotations

from services.jobs import update_job
from services.llm import (
    extract_case_strength, extract_statutes, extract_structured_analysis, extract_timeline,
)
from services.matters import load_matter, patch_matter, replace_matter_records
